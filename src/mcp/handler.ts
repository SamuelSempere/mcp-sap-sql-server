...
import { toolsListHandler } from '../tools/sqlTools';
...
export async function processMcpRequest(
  ...
  if (typeof (server as any).request === 'function') {
    try {
      return await (server as any).request(method, params);
    } catch (error) {
  ...
  if (typeof (server as any).processRequest === 'function') {
    try {
      return await (server as any).processRequest(request);
    } catch (error) {
      // Si falla por "Not connected", usar handlers directamente
      if (error instanceof Error && error.message.includes('connected')) {
        // Continuar con el fallback
  ...
  for (const prop of possibleHandlerProps) {
    if ((server as any)[prop]) {
      handlers = (server as any)[prop];
      if (process.env.DEBUG === 'true') {
  ...
  if (handlers) {
    ...
    if (method === 'tools/list') {
      ...
      if (handlers instanceof Map) {
        ...
        handler = handlers.get(ListToolsRequestSchema) || handlers.get('tools/list');
      ...
      if (handlers instanceof Map) {
        handler = handlers.get(CallToolRequestSchema) || handlers.get('tools/call');
      } else if (handlers.get && typeof handlers.get === 'function') {
        handler = handlers.get(CallToolRequestSchema) || handlers.get('tools/call');
      } else if (typeof handlers === 'object') {
