...
import { processMcpRequest } from './mcp/handler';
...
const API_KEY = config.auth.apiKey || process.env.MCP_API_KEY;
...
app.post('/mcp', authMiddleware, express.json(), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const response = await processMcpRequest(mcpServer, request);
    ...
    const errorCode = err instanceof Error && 'code' in err ? (err as any).code : -32603;
...
const certPath = process.env.CERT_PATH || path.join(__dirname, '..', 'cert');
...
if (USE_LOCAL_HTTPS && useHttps && httpsOptions) {
  ...
  httpsServer.listen(PORT, LISTEN_HOST, () => {
    ...
    console.log(`💾 Base de datos: ${config.sql.database}@${config.sql.host}:${config.sql.port}`);
  });
} else {
